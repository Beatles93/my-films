...
import { useNavigate } from "react-router-dom";
import loaderSvg from "../../assets/loader.svg";
import GenreSidebar from "../GenreSidebar/GenreSidebar"; // Импортируем новый компонент
...
function MovieList({ query }) {
  ...
  const getMovie = (query, genre, page = 1) => {
    const options = {
      method: "GET",
      headers: {
        accept: "application/json",
  ...
  const handlePageChange = (page) => {
    if (page > 0 && page <= totalPages) {
      setCurrentPage(page);
      getMovie(query, selectedGenre, page);
    }
  };

  useEffect(() => {
    setCurrentPage(1);
    getMovie(query, selectedGenre);
  }, [query, selectedGenre]);
  ...
  return (
    <div className={styles.movieListContainer}>
      <button onClick={() => setShowSidebar(!showSidebar)}>
        {showSidebar ? "Закрыть жанры" : "Открыть жанры"}
      </button>
      {showSidebar && <GenreSidebar onSelectGenre={setSelectedGenre} />}
      {loading && currentPage === 1 ? (
        <div className={styles.loaderContainer}>
          <img src={loaderSvg} alt="Loading..." className={styles.loader} />
        </div>
